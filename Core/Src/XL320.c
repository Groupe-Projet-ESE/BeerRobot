...
void servo(){
	...
	/*unsigned char LEN_LED[2]={0x06, 0x00};
	//LEN[0]=0x06; nbparam+3

	unsigned char PARAM_LED[3]={0x19,0x00,GREEN};

	//SEND LED ON COMMAND
	unsigned char TxPacket[13] = { HEADER0, HEADER1, HEADER2, RESERVED, ID, LEN_LED[0], LEN_LED[1], INSTR_WRITE, PARAM_LED[0], PARAM_LED[1], PARAM_LED[2], CRC_L, CRC_H};
	crc = update_crc(0, TxPacket, 11);
	CRC_L = (crc & 0x00FF);
	CRC_H = (crc>>8) & 0x00FF;
	TxPacket[11] = CRC_L;
	TxPacket[12] = CRC_H;
	HAL_UART_Transmit(&huart6, TxPacket, 13, 1000);
...
void clignote(){
	...
	unsigned char CRC_L = 0;
	unsigned char CRC_H = 0;
	unsigned char changecolor[8]={OFF, RED, GREEN , YELLOW, BLUE, PURPLE, CYAN, WHITE};

	for(int i=0; i<9; i++){
		//SEND LED ON COMMAND
		unsigned char TxPacket[13] = { 0xFF, 0xFF, 0xFD, 0x00, 0xFE, 0x06, 0x00, 0x03, 0x19, 0x00, changecolor[i], CRC_L, CRC_H};
		crc = update_crc(0, TxPacket, 11);
		CRC_L = (crc & 0x00FF);
		CRC_H = (crc>>8) & 0x00FF;
		TxPacket[11] = CRC_L;
		...
		HAL_UART_Transmit(&huart2, TxPacket, 13, 1000);
		HAL_Delay(1000);
	}
...
void close(){
	...
	TxPacket2[13] = CRC_H;
	HAL_UART_Transmit(&huart2, TxPacket2, 14, 1000);
	//Affichage trame envoyée à l'uart2 (action servomoteur pour fermeture)
	/*for (int i=0; i<14; i++){
		printf(" %x", TxPacket2[i]);
	}
	printf("\r\n");*/
...
void open(){
	...
	TxPacket2[13] = CRC_H;
	HAL_UART_Transmit(&huart2, TxPacket2, 14, 1000);
	//Affichage trame envoyée à l'uart2 (action servomoteur pour ouverture)
	/*for (int i=0; i<14; i++){
		printf(" %x", TxPacket2[i]);
	}
	printf("\r\n");*/
...
void speed(int speed){
	...
	TxPacket2[13] = CRC_H;
	HAL_UART_Transmit(&huart2, TxPacket2, 14, 1000);
	//Affichage trame envoyée à l'uart2 (vitesse servomoteur)
	/*for (int i=0; i<14; i++){
		printf(" %x", TxPacket2[i]);
	}
	printf("\r\n");*/
